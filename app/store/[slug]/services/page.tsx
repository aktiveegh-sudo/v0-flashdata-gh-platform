    ...
    <StoreShell store={store} slug={store.slug} activeTab="services">
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <section>
          ...
          <h2 className="mt-5 text-3xl font-black tracking-tight text-white sm:text-4xl">Digital services in {store.name}</h2>
          <p className="mt-4 max-w-xl text-sm leading-7 text-zinc-300">
            Access active services from this store with fast, clear pricing and checkout.
          </p>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            <div className="flex items-center gap-2 rounded-2xl border border-yellow-300/15 bg-zinc-900/80 px-4 py-3 text-sm text-zinc-200">
              <BadgeCheck className="h-4 w-4 text-yellow-300" /> Customer-facing only
            </div>
            <div className="flex items-center gap-2 rounded-2xl border border-yellow-300/15 bg-zinc-900/80 px-4 py-3 text-sm text-zinc-200">
              <BadgeCheck className="h-4 w-4 text-yellow-300" /> No main-site links
            </div>
          </div>
        </section>
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs uppercase tracking-[0.16em] text-zinc-400">{item.category}</p>
                  </div>
